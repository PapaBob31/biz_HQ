import electron, { ipcMain as ipcMain$1, BrowserWindow, app as app$1, safeStorage } from "electron";
...
const store = new ElectronStore({ name: "clover-vault" });
function encryptAndStoreTokens(tokens) {
...
ipcMain$1.handle("generate-pdf-report", async (event) => {
  ...
  require$$0$1.writeFileSync(filePath, data);
  return filePath;
});
ipcMain$1.handle("test-hardware", async (event, { action, printerName }) => {
...
ipcMain$1.handle("user-login", async (event, { username, password, role }) => {
  try {
